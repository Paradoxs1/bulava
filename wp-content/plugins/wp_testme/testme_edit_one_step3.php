...
print '<p>Чтобы удалить результат, просто удалите соотвествующий символ или баллы "от" и "до".</p>';
    ...
    if ($testme_results) {
        foreach ($testme_results as $result) {
            ...
            print "<p>Баллы от :
                                <input type='text' size='5' name='result_point_start[{$result->ID}]' value='{$result->result_point_start}'
                style='background-color:#e7ffe7' maxlength='5' />
                                до <input type='text' size='5' name='result_point_end[{$result->ID}]' value='{$result->result_point_end}'
                style='background-color:#e7ffe7' maxlength='5' />
                                </p>";
...
//Для нового теста, когда еще нет результатов
    ...
    Буква/Цифра</strong>: <input type="text" size="5" name="result_letter_new[]" value="" style="background-color:#e7ffe7" maxlength="5" />
    <?php } else {?>
    Баллы от : <input type="text" size="5" name="result_point_start_new[]" value="" style="background-color:#e7ffe7" maxlength="5" /> до <input type="text" size="5" name="result_point_end_new[]" value="" style="background-color:#e7ffe7" maxlength="5" />
    <?php } ?>
    </p>
